...
namespace MFST
{
	...
	bool Mfst::savestate()
	{
		...
		MFST_TRACE6("SAVESTATE:", storestate.size(), *(log.stream));
		return true;
	}
	bool Mfst::reststate()
	{
	...
	bool Mfst::start(Log::LOG log)
	{
		...
		case LENTA_END:
			...
			*(log.stream) << setw(4) << left << "Всего строк " << lenta_size << ", Синтаксический анализ выполнен без ошибок" << endl;
			...
			break;
		case SURPRISE:
			MFST_TRACE4("------> SURPRISE", *(log.stream))
				break;
		}
		return rc;
	}
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			Error::ERROR_ err = Error::geterrorin(errid, ERROR_ZERO_LINE, ERROR_ZERO_COL);
			//cout << "Ошибка " << err.id << ":" << err.message<< " строка " << lexTable.table[lpos].sn << endl ;
			throw ERROR_THROW_IN(err.id, lexTable.table[lpos].sn, lexTable.table[lpos].idxTI);
			rc = buf;
		}
		return rc;
	}
